...
async function callGeminiApi(
  ...
  if (!apiKey) {
    // This error is now server-side and will be logged, not shown to the user.
...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    ...
    const { base64File, mimeType } = JSON.parse(event.body);

    if (!base64File || !mimeType) {
      return { 
        statusCode: 400, 
        body: JSON.stringify({ error: 'Missing base64File or mimeType in request body.' }) 
      };
    }

    const csvData = await callGeminiApi(base64File, mimeType);
